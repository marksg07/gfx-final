...
Canvas2D::Canvas2D() :
    ...
    m_rayScene(nullptr)
{
    m_alphaLayer.resize(width() * height());
    m_canvasCopy.resize(width() * height());
}
...
void Canvas2D::mouseDown(int x, int y) {
    ...
    //std::cout << "Received mouseDown event at x: " << x << ", y: " << y << std::endl;
...
void Canvas2D::renderImage(Camera *camera, int width, int height) {
    if (m_rayScene) {
        ...
        m_rayScene->setDrawParams(camera, width, height);
        m_rayScene->draw(this);
